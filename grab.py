...
def main():    
    ...
    for page in tweepy.Cursor(api.home_timeline, count=200, tweet_mode="extended", since_id=since_id).pages(call_amount):
        for status in page:
            ...
            print(blockline + 'Checking tweet: ' + status.full_text + ' from ' + status.user.screen_name + ' at ' + str(status.created_at) + blockline)     
